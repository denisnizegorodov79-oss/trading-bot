...
class TradingAgent:
    ...
    def analyze_and_decide(self, signal_data):
        ...
        poc = details.get('poc', price)
        ema200 = details.get('ema200', price)
        ...
        ema_diff = (price - ema200) / ema200 if ema200 else 0
        trend = self._get_trend(ema_diff)
        pattern_key = f"{symbol}_{direction}_{rsi_range}_{trend}"
        
        success_rate = self.successful_patterns.get(pattern_key, 0)
        fail_rate = self.failed_patterns.get(pattern_key, 0)
        total = success_rate + fail_rate
        if total > 3:
            history_confidence = success_rate / total
            confidence = confidence * 0.6 + history_confidence * 0.4
    ...
    def save_trade_result(self, trade_data):
        self.learning_data.append(trade_data)
        key = f"{trade_data.get('symbol')}_{trade_data.get('direction')}_{trade_data.get('rsi_range')}_{trade_data.get('trend')}"
        if trade_data.get('result') == 'WIN':
            self.successful_patterns[key] = self.successful_patterns.get(key, 0) + 1
        else:
            self.failed_patterns[key] = self.failed_patterns.get(key, 0) + 1
        
        file_exists = os.path.exists(CSV_LEARNING)
        with open(CSV_LEARNING, 'a', newline='') as f:
...
class TradeManager:
    ...
    def open_trade(self, symbol, direction, price, sl, tp, decision):
        ...
        position_size = agent.calculate_position_size(price, sl)
        ...
        trade = {
            ...
            "rsi_range": decision.get("rsi_range"),
    ...
    def close_trade(self, trade, close_price, reason):
        ...
        learning_data = {
            ...
            "result": trade['status'],
            "pnl_pct": pnl_pct,
            "rsi_range": trade.get('rsi_range'),
            "trend": trade.get('trend'),
            "confidence": trade.get('decision_confidence'),
            "reason": trade.get('decision_reason'),
            "pattern_key": trade.get('pattern_key'),
            "timestamp": trade['close_time']
        }
        agent.save_trade_result(learning_data)
        return trade
...
def start_cmd(msg):
    ...
    bot.send_message(msg.chat.id,
        ...
        "Бот сам анализирует рынок, принимает решения и учится на своих ошибках.\n\n"
...
def settings_btn(msg):
    text = (f"⚙️ *Текущие настройки*\n\n"
            ...
            f"Изменить: /set max_daily_trades 8")
    bot.reply_to(msg, text, parse_mode='Markdown')
...
def set_param(msg):
    try:
        args = msg.text.split()
        if len(args) != 3:
            bot.reply_to(msg, "Использование: /set max_daily_trades 8")
            return
        key, val = args[1], args[2]
        if key == "max_daily_trades":
            CONFIG["max_daily_trades"] = int(val)
        elif key == "risk_per_trade":
            CONFIG["risk_per_trade"] = float(val)
        elif key == "min_confidence":
            CONFIG["min_confidence"] = float(val)
        else:
            bot.reply_to(msg, f"Неизвестный параметр: {key}")
            return
        bot.reply_to(msg, f"✅ Параметр {key} изменён на {val}")
    except Exception as e:
